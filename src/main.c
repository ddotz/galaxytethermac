...
static volatile int g_watch_enabled = 1;  /* start idle, UI sends "enable" if auto-connect is on */
...
static void restore_network_state(void)
{
    ...
    if (!g_no_dns) {
        /* Temporary service removal restores resolver selection. */
    }
    g_network_bound = 0;
}
                       ...
                       ipc_server_t *ipc)
{
    ...
    if (!no_dns) {
        ...
        /* Use only the temporary service DNS; preserve global DNS. */
...
int main(int argc, char **argv)
{
    ...
    /* Service uses only explicit root-owned configuration. */
    ...
    ipc_server_t *ipc = NULL; /* Local diagnostic build: no control socket. */
