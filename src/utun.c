...
static int owned_route_low = 0;
static int owned_route_high = 0;
static char owned_gateway[INET_ADDRSTRLEN];

int utun_set_default_route(utun_t *tun, const char *gateway)
{
    (void)tun;
    char cmd[256];
    strlcpy(owned_gateway, gateway, sizeof(owned_gateway));
    snprintf(cmd, sizeof(cmd), "/sbin/route add -net 0.0.0.0/1 %s", gateway);
    owned_route_low = system(cmd) == 0;
    snprintf(cmd, sizeof(cmd), "/sbin/route add -net 128.0.0.0/1 %s", gateway);
    owned_route_high = system(cmd) == 0;
    return owned_route_low && owned_route_high ? 0 : -1;
}

void utun_restore_routes(void)
{
    char cmd[256];
    if (owned_route_low) {
        snprintf(cmd, sizeof(cmd), "/sbin/route delete -net 0.0.0.0/1 %s 2>/dev/null", owned_gateway);
        system(cmd);
    }
    if (owned_route_high) {
        snprintf(cmd, sizeof(cmd), "/sbin/route delete -net 128.0.0.0/1 %s 2>/dev/null", owned_gateway);
        system(cmd);
    }
    owned_route_low = owned_route_high = 0;
}
